...
# Add component directories to build paths
env.Append(
    CPPPATH=[
        os.path.join(env.subst("$PROJECT_DIR"), "main/include"),
    ]
)

# Add all component include directories
for component in os.listdir(components_dir):
    component_include = os.path.join(components_dir, component, "include")
    if os.path.exists(component_include):
        env.Append(CPPPATH=[component_include])
        print(f"Added include path: {component_include}")
